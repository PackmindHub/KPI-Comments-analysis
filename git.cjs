...
// src/fetchData/agent/AbstractMergeRequestRepository.ts
var AbstractMergeRequestRepository = class {
  constructor(_api, _logger) {
    this._api = _api;
    this._logger = _logger;
  }
  async dumpMergeRequests(repositories, ignoreRequestBefore, includes) {
...
=== Processing repository: ${repo} ===`);
      ...
      this._logger.info(` \u2705 Repository ${repo}: found ${repoMergeRequests.length} merge requests`);
      this._logger.info(` \u{1F4DD} Applying include filters...`);
      const filteredMergeRequests = this.applyIncludeFilter(repoMergeRequests, includes);
      this._logger.info(` \u{1F4DD} Repository ${repo}: ${filteredMergeRequests.length} merge requests after include filter`);
    ...
    this.anonymizeGitUsers(parsedRepositories);
    this._logger.info(`
=== Summary ===
Total merge requests found across all repositories: ${totalMergeRequests}`);
    return parsedRepositories;
  }
  async extractMergeRequests(repo, page, ignoreRequestBefore) {
    this._logger.info(`\u231B Fetching page ${page + 1} for ${repo}...`);
    this.assertApi();
    const mergeRequestIds = await this.getPullRequests(repo, page + 1, ignoreRequestBefore);
    const mergeRequests = [];
    for (const mergeRequest of mergeRequestIds) {
      try {
        const files = await this.extractMergeRequestFiles(repo, mergeRequest);
        mergeRequests.push({
          files,
          ...mergeRequest
        });
      } catch (error) {
        this._logger.error(`\u274C Failed to process merge request: ${error}`);
        continue;
      }
    }
    return mergeRequests.length > 0 ? mergeRequests : null;
  }
  applyIncludeFilter(mergeRequests, includes) {
    return mergeRequests.reduce((acc, mr) => {
      const files = mr.files.filter((file) => {
        if (!file.discussions.length) {
          return false;
        } else if (!includes.length) {
          return true;
        }
        return includes.some((pattern) => minimatch(file.path, pattern));
      });
      if (files.length > 0) {
        acc.push({
          ...mr,
  ...
  anonymizeGitUsers(repositories) {
    const records = /* @__PURE__ */ new Map();
    this._logger.info(`\u{1F513} Anonymizing git users...`);
    repositories.forEach(
      (repository) => repository.mergeRequests.forEach(
        (mr) => {
          mr.files.forEach((file) => {
            file.discussions.forEach((discussion) => {
              discussion.messages.forEach((message) => {
                if (!records.has(message.author)) {
                  records.set(message.author, `User${records.size + 1}`);
                }
                message.author = records.get(message.author);
              });
            });
          });
        }
    ...
    this._logger.info(`\u{1F512} Anonymized ${records.size} git users`);
  }
};

// src/fetchData/agent/GitlabMergeRequestRepository.ts
var GITLAB_MAX_REQUESTS_PER_MINUTE = 300;
var DELAY_BETWEEN_REQUESTS_IN_MS = 60 * 1e3 / GITLAB_MAX_REQUESTS_PER_MINUTE;
var delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
var GitLabMergeRequestsRepository = class extends AbstractMergeRequestRepository {
  constructor(gitCredentials, noDiff, logger) {
    super(new GitLabAPI(
      {
        apiUrl: `${gitCredentials.hostname}/api/v4`,
  ...
  assertApi() {
    if (!this._api) {
      this._logger.error("GitlabMergeRequestRepository has not been initialized.");
      throw new Error("GitlabMergeRequestRepository has not been initialized.");
    }
  }
  async getPullRequests(repo, page, ignoreRequestBefore) {
    const mergeRequests = await this._api.getMergeRequests(repo, page);
    return mergeRequests.reduce((acc, pr) => {
  ...
  async extractMergeRequestFiles(repo, mergeRequest) {
    ...
    for (const [path3, gitLabDiscussionsForFile] of Object.entries(groupedDiscussions)) {
      ...
      for (const discussion of gitLabDiscussionsForFile) {
        ...
        if (firstNote?.position) {
          const timestamp = new Date(firstNote.created_at);
          const diffs = await this.getDiffs(repo, firstNote.position.base_sha, firstNote.position.head_sha);
          const newPath = firstNote.position.new_path;
          const diff = diffs.find((d) => d.new_path === newPath)?.diff;
          const messages = discussion.notes.filter((note) => this.filterCommentsFromHumansOnly(note.author.username)).map((note) => ({
            id: note.id,
    ...
    return mergeRequestFiles;
  }
  filterCommentsFromHumansOnly(username) {
  ...
  async getDiffs(repo, baseSha, headSha) {
    ...
    if (!this.diffCache[cacheKey]) {
      this.diffCache[cacheKey] = await this._api.getFileDiff(repo, baseSha, headSha);
    }
  ...
  groupDiscussions(discussions) {
    ...
    for (const discussion of discussions) {
      const filePath = discussion.notes[0]?.position?.old_path;
      if (filePath) {
        if (!groupedDiscussions[filePath]) {
          groupedDiscussions[filePath] = [];
        }
        groupedDiscussions[filePath].push(discussion);
      }
    }
...
var GitLabAPI = class {
  constructor(_connectionData, _logger) {
    ...
    this._connection.interceptors.request.use(async (config) => {
      const now = Date.now();
      const timeSinceLastRequest = now - latestRequestTime;
      if (timeSinceLastRequest < DELAY_BETWEEN_REQUESTS_IN_MS) {
        await delay(DELAY_BETWEEN_REQUESTS_IN_MS - timeSinceLastRequest);
      }
      latestRequestTime = now;
      return config;
    });
  }
  async getMergeRequests(repo, page) {
    const url2 = `${this._connectionData.apiUrl}/projects/${encodeURIComponent(repo)}/merge_requests/?state=merged&order=desc&order_by=updated_at&scope=all&per_page=100&page=${page}`;
    this._logger.info(`  \u231B Fetching page ${page} for merge requests`);
    try {
      const { data } = await this._connection.get(url2, {
        headers: this.getHeaders()
      });
      this._logger.info(`  \u2705 Retrieved ${data.length} merge requests from page ${page}`);
      return data;
    } catch (err) {
      const message = err.message || err?.message || JSON.stringify(err);
      this._logger.error(`\u274C Error querying merge requests: "${message}"`);
      throw new Error(message);
    }
  }
  async getMergeRequestDiscussion(repo, mergeRequestId) {
    const url2 = `${this._connectionData.apiUrl}/projects/${encodeURIComponent(repo)}/merge_requests/${encodeURIComponent(mergeRequestId)}/discussions`;
    this._logger.info(`    \u231B Fetching discussions for MR #${mergeRequestId}`);
    try {
      const { data } = await this._connection.get(url2, {
        headers: this.getHeaders()
      });
      ...
      const message = err.message || err?.message || JSON.stringify(err);
      this._logger.error(`    \u274C Error fetching discussions: ${message}`);
      throw new Error(message);
    }
  }
  async getFileDiff(repo, from, to) {
    ...
    try {
      ...
      if (status !== 200) {
        this._logger.error(`      \u274C Failed to fetch diff: status ${status}`);
        return [];
      }
      this._logger.info(`      \u2705 Retrieved ${data.diffs?.length || 0} file diffs`);
      return data.diffs;
    } catch (err) {
      const message = err.message || err?.message || JSON.stringify(err);
  ...
  getHeaders() {
    return {
      "Private-Token": this._connectionData.token
    };
  }
};
...
function initDataFetcher(config, logger) {
  switch (config.gitProvider) {
    case "gitlab" /* GITLAB */:
      return new GitLabMergeRequestsRepository(
        {
          token: config.gitToken,
          provider: config.gitProvider,
          hostname: config.gitUrl || "https://gitlab.com"
        },
        config.noDiff,
        logger
      );
...
function createLogger(verbose) {
  return verbose ? new VerboseLogger() : new SilentLogger();
}
...
function parseConfig() {
  ...
  return {
    ...
    includes: options.includes ? options.includes.split(",").filter((include) => include.length > 0) : []
  };
}
function showConfig(logger, config) {
  const { gitToken, ...rest } = config;
  logger.info("Fetching merge requests with following config:");
  logger.info(JSON.stringify({ ...rest, gitToken: "***" }, null, 2));
}
async function main() {
